WITH CASE_TABLE AS (
 -- Collect all the open Default 1,2,4,5 and Escalation Cases in Salesforce --
  ...
  WHERE
      ...
      C.RECORD_TYPE IN ('Solar - Service','Solar - Troubleshooting') OR (C.RECORD_TYPE = 'Solar - Customer Escalation' AND C.EXECUTIVE_RESOLUTIONS_ACCEPTED IS NULL)
  ) OR (
...
SELECT
    CT.*
    , CH.CREATEDDATE
    , DATEDIFF(S,CREATEDDATE,NVL(LEAD(CREATEDDATE) OVER (PARTITION BY CT.CASE_NUMBER ORDER BY CH.CREATEDDATE),CURRENT_TIMESTAMP()))/(24*60*60) AS GAP
...
FULL_CASE AS (
    SELECT
        ...
        , CASE
            WHEN STATUS1 IN ('In Progress') AND DEFAULT_BUCKET1 NOT IN ('MBW','Pending Legal') THEN 'P4/P5/DRA Letters'
            WHEN STATUS1 IN ('Pending Corporate Action','Pending Customer Action') AND DEFAULT_BUCKET1 NOT IN ('MBW','Pending Legal') THEN 'Working with Customer'
...
ACTIVE_CODES AS (
    ...
    WHERE
        LD.COLLECTION_CODE IN ('BK07','BK13','FORE','CORP')        
),

LD_FULL_HISTORY AS (
    SELECT
          LDF.BILLING_ACCOUNT AS BILLING_ACCOUNT_H
        , NVL(LDH.COLLECTION_CODE, 'NULL') AS COLLECTION_CODE1
        , IFF(LDH.COLLECTION_CODE LIKE '%BK%','BK',COLLECTION_CODE1) AS LD_CODE
        , NVL(LDH.COLLECTION_DATE,NVL(LAG(LDH.AS_OF_DATE) OVER (PARTITION BY LDF.BILLING_ACCOUNT ORDER BY LDH.COLLECTION_DATE),LDH.AS_OF_DATE)) AS CODE_WIP_START1
        , LEAD(LDH.COLLECTION_DATE) OVER (PARTITION BY LDF.BILLING_ACCOUNT ORDER BY LDH.COLLECTION_DATE) AS CODE_WIP_END1
    FROM
        LD_HISTORY_FLAG AS LDF
    LEFT JOIN
        LD.T_DAILY_DATA_EXTRACT_HIST AS LDH
...
LD_CODES AS (
SELECT
    BILLING_ACCOUNT_H AS BILLING_ACCOUNT
...
CORP_MERGE_TABLE AS (
    SELECT
        ...
        , CASE
            ...
            WHEN ((C.RECORD_TYPE1 = 'Solar - Customer Escalation' AND C.ERA IS NOT NULL) OR C.RECORD_TYPE1 = 'Solar - Cancellation Request') AND LD.LD_CODE = 'CORP' THEN 'CORP-ER'
            ...
            WHEN (C.RECORD_TYPE1 IN ('Solar - Service','Solar - Troubleshooting') OR (C.RECORD_TYPE1 = 'Solar - Customer Escalation' AND C.ERA IS NULL)) AND LD.LD_CODE = 'CORP' THEN 'CORP-CX'
            ELSE LD.LD_CODE
            END AS CORP_BUCKET
        , CASE
...
SOLUTIONS_WORKBOOK AS (
    ...
    FROM
        CORP_MERGE_TABLE
    WHERE
        MASTER_BUCKET IS NOT NULL
),

DAILY_WIP AS (
	SELECT
		  D.DT
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) THEN 1 END) AS ALL_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'Pending Legal' THEN 1 END) AS Pending_Legal_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'D1' THEN 1 END) AS D1_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'D2' THEN 1 END) AS D2_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'D4' THEN 1 END) AS D4_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'D5' THEN 1 END) AS D5_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'MBW' THEN 1 END) AS MBW_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'BK' THEN 1 END) AS BK_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'FORE' THEN 1 END) AS FORE_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'CORP-ER' THEN 1 END) AS CORP_ER_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'CORP-Default' THEN 1 END) AS CORP_Default_WIP
		, SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'CORP-Damage' THEN 1 END) AS CORP_Damage_WIP
	    , SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'CORP-CX' THEN 1 END) AS CORP_CX_WIP
	    , SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'CORP-Removal & Reinstall' THEN 1 END) AS CORP_RR_WIP
	    , SUM(CASE WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI =1) AND SW.MASTER_BUCKET = 'CORP-No Case' THEN 1 END) AS CORP_NO_CASE_WIP
	FROM
		SOLUTIONS_WORKBOOK AS SW
		, RPT.T_DATES AS D
	WHERE
		D.DT BETWEEN DATEADD('y',-1,DATE_TRUNC('MM',CURRENT_DATE())) AND CURRENT_DATE()
	GROUP BY
		D.DT
	ORDER BY
		D.DT
...
SELECT
    ...
    , FORE_WIP
    , BK_WIP
    , D1_WIP + D2_WIP + D4_WIP + D5_WIPP + BK_WIP + FORE_WIP + CORP_DEFAULT_WIP + CORP_NO_CASE_WIP AS TOTAL
FROM
	DAILY_WIP AS DW
WHERE
	DW.DT = CURRENT_DATE()
	OR
	DW.DT = LAST_DAY(DW.DT)