WITH CASE_TABLE AS (
   ...
   , CORE_TABLE AS (
    SELECT DATE_TRUNC('MM', D.DT)                                AS MONTH1
         , MASTER_BUCKET
         , COUNT(CASE WHEN TO_DATE(WIP_START) = D.DT THEN 1 END) AS INFLOW
         , COUNT(CASE WHEN TO_DATE(WIP_END) = D.DT THEN 1 END)   AS OUTFLOW
   ...
   , I_TABLE AS (
    SELECT MONTH1
         ...
         , SUM(CASE WHEN MASTER_BUCKET = 'CORP-No Case' THEN INFLOW END) AS NOCASE_INFLOW
         , SUM(CASE WHEN MASTER_BUCKET = 'CORP-Default' THEN INFLOW END) AS DEFAULT_INFLOW
         , SUM(CASE WHEN MASTER_BUCKET = 'D1' THEN INFLOW END)           AS D1_INFLOW
         , SUM(CASE WHEN MASTER_BUCKET = 'D2' THEN INFLOW END)           AS D2_INFLOW
         , SUM(CASE WHEN MASTER_BUCKET = 'D4' THEN INFLOW END)           AS D4_INFLOW
         , SUM(CASE WHEN MASTER_BUCKET = 'D5' THEN INFLOW END)           AS D5_INFLOW
         , SUM(CASE WHEN MASTER_BUCKET = 'FORE' THEN INFLOW END)         AS FORE_INFLOW
         , SUM(CASE WHEN MASTER_BUCKET = 'BK' THEN INFLOW END)           AS BK_INFLOW
         , BK_INFLOW + FORE_INFLOW + NOCASE_INFLOW + DEFAULT_INFLOW + D1_INFLOW + D2_INFLOW + D4_INFLOW +
           D5_INFLOW                                                     AS TOTAL_INFLOW
    FROM CORE_TABLE AS CT
    WHERE MASTER_BUCKET IN ('BK', 'CORP-Default', 'CORP-No Case', 'D1', 'D2', 'D4', 'D5', 'FORE')
    GROUP BY 1
    ORDER BY 1
)
