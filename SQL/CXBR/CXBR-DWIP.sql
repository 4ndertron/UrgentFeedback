WITH CASE_TABLE AS (
   ...
   , CORP_MERGE_TABLE AS (
    SELECT C.*
         ...
         , CASE
               ...
               WHEN C.RECORD_TYPE1 = 'Solar - Panel Removal' AND LD.LD_CODE = 'CORP' THEN 'CORP-Removal & Reinstall'
   ...
   , DAILY_WIP AS (
    SELECT D.DT
         ...
         , SUM(CASE
                   WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI = 1) AND
                        SW.MASTER_BUCKET = 'Pending Legal' THEN 1 END)            AS Pending_Legal_WIP
         , SUM(CASE
                   WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI = 1) AND
                        SW.MASTER_BUCKET = 'D1' THEN 1 END)                       AS D1_WIP
         , SUM(CASE
                   WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI = 1) AND
                        SW.MASTER_BUCKET = 'D2' THEN 1 END)                       AS D2_WIP
         , SUM(CASE
                   WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI = 1) AND
                        SW.MASTER_BUCKET = 'D4' THEN 1 END)                       AS D4_WIP
         , SUM(CASE
                   WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI = 1) AND
                        SW.MASTER_BUCKET = 'D5' THEN 1 END)                       AS D5_WIP
         , SUM(CASE
                   WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI = 1) AND
                        SW.MASTER_BUCKET = 'MBW' THEN 1 END)                      AS MBW_WIP
         , SUM(CASE
                   WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI = 1) AND
                        SW.MASTER_BUCKET = 'BK' THEN 1 END)                       AS BK_WIP
         , SUM(CASE
                   WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI = 1) AND
                        SW.MASTER_BUCKET = 'FORE' THEN 1 END)                     AS FORE_WIP
         , SUM(CASE
                   WHEN TO_DATE(WIP_START) <= D.DT AND (TO_DATE(WIP_END) >= D.DT OR WIP_END IS NULL OR WIP_KPI = 1) AND
   ...
   , FINAL_WIP AS (
    SELECT DT
         ...
         , D1_WIP
         , D2_WIP
         , NVL(D4_WIP, 0)                                                                                AS D4_WIPP
         , NVL(D5_WIP, 0)                                                                                AS D5_WIPP
         , FORE_WIP
         , BK_WIP
         , D1_WIP + D2_WIP + D4_WIPP + D5_WIPP + BK_WIP + FORE_WIP + CORP_DEFAULT_WIP + CORP_NO_CASE_WIP AS TOTAL
