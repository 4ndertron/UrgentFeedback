WITH T1 AS (
    ...
    SELECT D.DT
         ...
         , SUM(CASE
                   WHEN T3.ER_ACCEPTED_DAY <= D.DT AND (T3.DAY_CLOSED >= D.DT OR T3.DAY_CLOSED IS NULL) AND
                        T3.PRIORITY_BUCKET = 'Executive/News Media' THEN 1 END) AS EXECUTIVE_WIP
         , SUM(CASE
                   WHEN T3.ER_ACCEPTED_DAY <= D.DT AND (T3.DAY_CLOSED >= D.DT OR T3.DAY_CLOSED IS NULL) AND
   ...
   , WIP_TABLE AS (
    SELECT DT
         ...
         , CREDIT_WIP
         , INTERNAL_WIP
         , EXECUTIVE_WIP + LEGAL_WIP AS OVERVIEW_TOTAL
    FROM T4
    WHERE DT = CURRENT_DATE
       OR DT = LAST_DAY(T4.DT)
)
