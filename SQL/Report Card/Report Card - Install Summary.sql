WITH PROJECTS_RAW AS (
   ...
   , INSTALL_ION AS (
    SELECT D.DT
         ...
         , PR.STATE_NAME
         , PR.ORG_BUCKET
         , SUM(COUNT(CASE WHEN PR.INSTALLATION_DATE = D.DT THEN 1 END)) OVER
        (PARTITION BY PR.STATE_NAME, PR.ORG_BUCKET
        ORDER BY D.DT ROWS BETWEEN 30 PRECEDING AND 1 PRECEDING) AS INSTALL_INFLOW
         , SUM(COUNT(CASE WHEN PR.CANCELLATION_DATE = D.DT THEN 1 END)) OVER
        (PARTITION BY PR.STATE_NAME, PR.ORG_BUCKET
        ORDER BY D.DT ROWS BETWEEN 30 PRECEDING AND 1 PRECEDING) AS INSTALLATION_OUTFLOW
         , INSTALL_INFLOW - INSTALLATION_OUTFLOW                 AS INSTALLATION_NET
         , SUM(COUNT(CASE WHEN PR.IN_SERVICE_DATE = D.DT THEN 1 END)) OVER
        (PARTITION BY PR.STATE_NAME, PR.ORG_BUCKET
        ORDER BY D.DT ROWS BETWEEN 30 PRECEDING AND 1 PRECEDING) AS PRE_PTO_OUTFLOW
