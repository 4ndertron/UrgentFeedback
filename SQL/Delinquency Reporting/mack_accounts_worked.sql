--Unique Default Manager Touches for Accounts Worked Summary
(select distinct u.name                        names,
 ...
 where (c.subject ilike 'D3%'
   ...
   and date_trunc(day, s.createdate) between
     to_date(dateadd(day, -90, current_date)) and
...
(select t.created_by                    names,
 ...
 from rpt.t_task t
 where date_trunc(day, t.created_date) between to_date(dateadd(day, -90, current_date)) and dateadd(day, -1, current_date)
