...
T4 AS (
    SELECT
           D.DT
         , D.WEEK_DAY_NUM
         , SUM(CASE WHEN T3.ER_ACCEPTED_DAY <= D.DT AND (T3.DAY_CLOSED >= D.DT OR T3.DAY_CLOSED IS NULL) THEN 1 END) AS ALL_WIP
         , SUM(CASE WHEN T3.ER_ACCEPTED_DAY <= D.DT AND (T3.DAY_CLOSED >= D.DT OR T3.DAY_CLOSED IS NULL) AND T3.PRIORITY_BUCKET = 'Executive/News Media' THEN 1 END) AS P1_WIP
         , SUM(CASE WHEN T3.ER_ACCEPTED_DAY <= D.DT AND (T3.DAY_CLOSED >= D.DT OR T3.DAY_CLOSED IS NULL) AND T3.PRIORITY_BUCKET = 'Legal/BBB' THEN 1 END) AS P2_WIP
         , SUM(CASE WHEN T3.ER_ACCEPTED_DAY <= D.DT AND (T3.DAY_CLOSED >= D.DT OR T3.DAY_CLOSED IS NULL) AND T3.PRIORITY_BUCKET = 'Online Review' THEN 1 END) AS P3_WIP
         , SUM(CASE WHEN T3.ER_ACCEPTED_DAY <= D.DT AND (T3.DAY_CLOSED >= D.DT OR T3.DAY_CLOSED IS NULL) AND T3.PRIORITY_BUCKET = 'Social Media' THEN 1 END) AS P4_WIP
         , SUM(CASE WHEN T3.ER_ACCEPTED_DAY <= D.DT AND (T3.DAY_CLOSED >= D.DT OR T3.DAY_CLOSED IS NULL) AND T3.PRIORITY_BUCKET = 'Internal' THEN 1 END) AS P5_WIP
    FROM
        T3
        ,RPT.T_DATES AS D
    WHERE
        D.DT BETWEEN DATEADD('y',-1,DATE_TRUNC('month',CURRENT_DATE())) AND CURRENT_DATE()
    GROUP BY 
          D.DT
        , D.WEEK_DAY_NUM
    ORDER BY
...
SELECT
    ...
    , P2_WIP AS LEGAL_WIP
    , EXECUTIVE_WIP + LEGAL_WIP AS TOTAL_WIP
FROM
    T4
WHERE
    T4.DT = CURRENT_DATE
    OR
    T4.DT = LAST_DAY(T4.DT)