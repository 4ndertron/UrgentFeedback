WITH AGENTS AS (
   ...
   , WIP_TABLE AS (
    SELECT D.DT
         , SUM(CASE
                   WHEN TO_DATE(SW.CREATED_DATE) <= D.DT AND
                        (TO_DATE(SW.CLOSED_DATE) >= D.DT OR TO_DATE(SW.CLOSED_DATE) IS NULL)
                       THEN 1 END)                                 AS CASE_WIP
         , ROUND(AVG(CASE
                         WHEN TO_DATE(SW.CREATED_DATE) <= D.DT AND
                              (TO_DATE(SW.CLOSED_DATE) >= D.DT OR TO_DATE(SW.CLOSED_DATE) IS NULL)
                             THEN SW.CASE_AGE END), 2)             AS AVG_ACTIVE_AGE
         , ROUND(AVG(CASE
                         WHEN TO_DATE(SW.CREATED_DATE) <= D.DT AND
                              (TO_DATE(SW.CLOSED_DATE) >= D.DT OR TO_DATE(SW.CLOSED_DATE) IS NULL)
                             THEN SW.LAST_30_DAY_COVERAGE END), 2) AS AVG_COVERAGE
   ...
   , ION_TABLE AS (
    SELECT IFF(LAST_DAY(D.DT) >= CURRENT_DATE(), CURRENT_DATE(), LAST_DAY(D.DT))     AS MONTH_1
         , COUNT(CASE WHEN TO_DATE(SW.CREATED_DATE) = D.DT THEN 1 END)               AS CASES_CREATED
         , COUNT(CASE WHEN TO_DATE(SW.CLOSED_DATE) = D.DT THEN 1 END)                AS CASES_CLOSED
         , CASES_CREATED - CASES_CLOSED                                              AS NET
         , ROUND(AVG(CASE WHEN TO_DATE(SW.CLOSED_DATE) = D.DT THEN CASE_AGE END), 2) AS AVERAGE_CLOSED_AGE
    FROM SOLUTIONS_WORKBOOK AS SW
       , RPT.T_DATES AS D
   ...
   , MERGE_TABLE AS (
    SELECT WT.DT                 AS D_KPI_DT
         , WT.CASE_WIP           AS D_KPI_CASE_WIP
         , WT.AVG_COVERAGE       AS D_KPI_AVG_COVERAGE
         , IT.CASES_CLOSED       AS D_KPI_CASES_CLOSED
         , IT.AVERAGE_CLOSED_AGE AS D_KPI_AVERAGE_CLOSED_AGE
