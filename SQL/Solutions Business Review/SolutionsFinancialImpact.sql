    ...
    ORDER BY C.PROJECT_ID
     ...
     FINAL AS (
         SELECT DATE_TRUNC('MM', D.DT)                                                          AS MONTH1
              , MASTER_BUCKET
              , NVL(SUM(CASE
                            WHEN TO_DATE(WIP_END) = D.DT AND RECORD_TYPE1 = 'Solar - Customer Default' AND
                                 STATUS1 = 'Closed - Saved' THEN NVL(SYSTEM_VALUE, 0) END), 0)  AS VALUE_SECURED
              , NVL(COUNT(CASE
                              WHEN TO_DATE(WIP_END) = D.DT AND RECORD_TYPE1 = 'Solar - Customer Default' AND
                                   STATUS1 = 'Closed - Saved' THEN SYSTEM_VALUE END), 0)        AS VOLUME_SECURED
              , NVL(SUM(CASE
                            WHEN TO_DATE(WIP_END) = D.DT AND RECORD_TYPE1 = 'Solar - Customer Default' AND
                                 STATUS1 = 'Closed'
                                THEN NVL(SYSTEM_VALUE, 0) END), 0)                              AS VALUE_LOST
              , NVL(COUNT(CASE
                              WHEN TO_DATE(WIP_END) = D.DT AND RECORD_TYPE1 = 'Solar - Customer Default' AND
                                   STATUS1 = 'Closed'
                                  THEN SYSTEM_VALUE END), 0)                                    AS VOLUME_LOST
              , NVL(SUM(CASE
                            WHEN TO_DATE(WIP_END) = D.DT AND RECORD_TYPE1 = 'Solar - Customer Default' AND
                                 STATUS1 NOT LIKE '%Closed%' THEN NVL(SYSTEM_VALUE, 0) END), 0) AS VALUE_PENDING
              , NVL(COUNT(CASE
                              WHEN TO_DATE(WIP_END) = D.DT AND RECORD_TYPE1 = 'Solar - Customer Default' AND
                                   STATUS1 NOT LIKE '%Closed%' THEN SYSTEM_VALUE END), 0)       AS VOLUME_PENDING
              , NVL(VALUE_SECURED, 0) - NVL(VALUE_LOST, 0)                                      AS NET_VALUE
              , NVL(VOLUME_SECURED, 0) - NVL(VOLUME_LOST, 0)                                    AS NET_VOLUME
...
SELECT MONTH1              AS SELECTED_MONTH
     , SUM(VOLUME_SECURED) AS VOLUME_SAVED
FROM FINAL
WHERE MONTH1 BETWEEN DATE_TRUNC('Y', CURRENT_DATE()) AND LAST_DAY(DATEADD('MM', -2, CURRENT_DATE()))
GROUP BY MONTH1
ORDER BY MONTH1