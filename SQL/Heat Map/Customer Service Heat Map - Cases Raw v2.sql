-- v2: Added System Status, Past Due Balance, and Latest Comment
    ...
    (
        SELECT pr.roc_name
             ...
             , RTRIM(
                    ...
                    (CASE WHEN ca.solar_queue = 'Tier II' AND pa4.case_id IS NOT NULL THEN 'Service Report Card - Billing Information.sql, ' END) ||
...
SELECT ca.roc_name
     ...
     , REPLACE(REPLACE(REPLACE(ca.latest_comment, CHR(9), ' '), CHR(10), ' '), CHR(13), ' ') AS latest_comment
...
FROM (
     ...
     ON
         ...
         LEFT OUTER JOIN
     mack_damavandi.mv_stable_systems ss
     ON
         se.project_id = ss.project_id
         LEFT OUTER JOIN
     ar.t_ld_daily_data_extract ar
     ON
         se.solar_billing_account_number = ar.billing_account
