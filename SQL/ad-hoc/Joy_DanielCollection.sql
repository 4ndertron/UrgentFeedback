...
--SELECT * FROM ACCOUNT_LIST
     T1 AS (
         SELECT ANY_VALUE(A.ACCOUNT_ID)                                       AS ACCOUNT_ID
              , ANY_VALUE(A.SALES_REP_NAME)                                   AS SALES_REP_NAME
              , ANY_VALUE(A.SALES_CLOSER_NAME)                                AS SALES_CLOSER_NAME
              , A.SERVICE_NAME
              , ANY_VALUE(A.PROJECT_ID)                                       AS PROJECT_ID
              , ANY_VALUE(A.SERVICE_NUMBER)                                   AS SERVICE_NUMBER
              , ANY_VALUE(A.SERVICE_STATUS)                                   AS SERVICE_STATUS
              , ANY_VALUE(A.INSTALL_DATE)                                     AS INSTALL_DATE
              , ANY_VALUE(A.PTO_AWARDED)                                      AS PTO_AWARDED
              , ANY_VALUE(A.SYSTEM_SIZE)                                      AS SYSTEM_SIZE
              , COUNT(CASE WHEN C.RECORD_TYPE LIKE '%Default%' THEN 1 END)    AS DEFAULT_TALLY
              , COUNT(CASE WHEN C.RECORD_TYPE LIKE '%Escalation%' THEN 1 END) AS ESCALATION_TALLY
              , COUNT(CASE WHEN C.ORIGIN LIKE '%BBB%' THEN 1 END)             AS BBB_TALLY
...
SELECT SALES_REP_NAME
     ...
     , ROUND(SUM(CASE WHEN SERVICE_STATUS = 'Cancelled' AND INSTALL_DATE IS NOT NULL THEN (SYSTEM_SIZE * 1000) * 4 END),
             2)                                                                              AS COST_OF_INSTALL_CANCELLATION
     , SUM(DEFAULT_TALLY)                                                                    AS DEFAULT_TOTAL
     , SUM(ESCALATION_TALLY)                                                                 AS ESCALATION_TOTAL
     , SUM(BBB_TALLY)                                                                        AS BBB_TOTAL
FROM T1
