WITH AGENTS AS (
   ...
   , CANCELLED AS (
    SELECT P.PROJECT_NUMBER
         , ANY_VALUE(CANCELLATION_DATE)   AS CANCELLATION_DATE
...
--       AND STATUS != 'In Dispute'
   ...
   , MERGE AS (
    SELECT T3.CREATED_DATE
         ...
         , NVL(SUM(CASE WHEN C.CANCELLATION_DATE >= T3.CREATED_DATE THEN C.SYSTEM_VALUE ELSE 0 END),
               0)                                                                                    AS CANCELLATION_COSTS
...
--     WHERE C.CANCELLATION_DATE <= T3.CLOSED_DATE
...
SELECT DATE_TRUNC('MM', D.DT)                        AS MONTH_1
     , NVL(SUM(CASE
                   WHEN TO_DATE(C.CLOSED_DATE) = D.DT AND PRIORITY_BUCKET = 'Executive/News Media'
                       THEN ESCALATION_COST END), 0) AS EXECUTIVE_ESCALATION_COST
     , NVL(SUM(CASE
                   WHEN TO_DATE(C.CLOSED_DATE) = D.DT AND PRIORITY_BUCKET = 'Legal/BBB'
                       THEN ESCALATION_COST END), 0) AS LEGAL_ESCALATION_COST
     , NVL(SUM(CASE
                   WHEN TO_DATE(C.CLOSED_DATE) = D.DT AND PRIORITY_BUCKET = 'Online Review'
                       THEN ESCALATION_COST END), 0) AS REVIEW_ESCALATION_COST
     , NVL(SUM(CASE
                   WHEN TO_DATE(C.CLOSED_DATE) = D.DT AND PRIORITY_BUCKET = 'Social Media'
                       THEN ESCALATION_COST END), 0) AS SOCIAL_ESCALATION_COST
     , NVL(SUM(CASE
                   WHEN TO_DATE(C.CLOSED_DATE) = D.DT AND PRIORITY_BUCKET = 'Internal'
                       THEN ESCALATION_COST END), 0) AS INTERNAL_ESCALATION_COST
     , EXECUTIVE_ESCALATION_COST + LEGAL_ESCALATION_COST + REVIEW_ESCALATION_COST + SOCIAL_ESCALATION_COST +
       INTERNAL_ESCALATION_COST                      AS ESCALATION_COST
