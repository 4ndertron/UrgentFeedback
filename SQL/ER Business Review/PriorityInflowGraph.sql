WITH AGENTS AS (
   ...
   , T1 AS (
    SELECT C.CASE_NUMBER
         ...
         , C.PROJECT_ID                                                                            AS PID
         , CASE
               WHEN C.RECORD_TYPE LIKE '%Dama%' THEN 'Damage'
        ...
        END                                                                                        AS PRIORITY_TABLE
         ...
         , CC.CREATEDBYID
         , CASE WHEN c.CREATED_DATE IS NOT NULL AND c.CLOSED_DATE IS NULL THEN 1 END               AS WIP_kpi
         , CASE
               ...
               ELSE DATEDIFF('S', C.CREATED_DATE, nvl(cc.CREATEDATE, CURRENT_TIMESTAMP())) /
                    (60 * 60) END                                                                  AS HOURLY_RESPONSE_TAT
    ...
    WHERE (RECORD_TYPE = 'Solar - Customer Escalation'
        ...
        AND ORIGIN != 'NPS')
       OR RECORD_TYPE = 'Solar Damage Resolutions'
...
SELECT DATE_TRUNC('MM', D.DT)                                     AS P_MONTH_1
     , C.PRIORITY_BUCKET                                          AS P_PRIORITY_BUCKET
     , C.PRIORITY_CASE_BUCKET                                     AS P_PRIORITY_CASE_BUCKET
     , COUNT(CASE WHEN TO_DATE(C.CREATED_DATE) = D.DT THEN 1 END) AS P_PRIORITY_CREATED
     , COUNT(CASE WHEN TO_DATE(C.CLOSED_DATE) = D.DT THEN 1 END)  AS P_PRIORITY_CLOSED
     , P_PRIORITY_CREATED - P_PRIORITY_CLOSED                     AS P_NET
...
GROUP BY P_MONTH_1
       , P_PRIORITY_BUCKET
       , P_PRIORITY_CASE_BUCKET
ORDER BY P_PRIORITY_BUCKET
       , P_PRIORITY_CASE_BUCKET
       , P_MONTH_1