...
WITH CALL_VIEW AS (
   ...
   , CASE_COMMENT_VIEW AS (
    ...
    FROM RPT.V_SF_CASECOMMENT AS CC
             ...
             LEFT OUTER JOIN RPT.T_PAYMENT AS PMT
                             ON PMT.CASE_ID = CC.PARENTID
   ...
   , TEST_CTE AS (
    SELECT COUNT(SESSION_ID)          AS CALL_RECORDS
         ...
         , SUM(TV_SALES_TALLY)        AS TV_SALES_CALLS
         , COUNT(CCV_EMPLOYEE_ID)     AS CCV_EMPLOYEE_CT
         , SUM(CCV_SALES_TALLY)       AS CCV_SALES_TALLY
    FROM MERGE
)
...
SELECT MAX(SALES_CALL_RATIO)
     , AVG(SALES_CALL_RATIO)
     , MEDIAN(SALES_CALL_RATIO)
FROM MAIN